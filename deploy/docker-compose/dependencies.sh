...
function cmd_up() {
  (cd $self_dir
  docker-compose --file "$self_dir/dependencies.yaml" --project-name pgcqrs_deps up --detach --remove-orphans
  )
}
...
function cmd_down() {
  (cd $self_dir
  docker-compose --file "$self_dir/dependencies.yaml" --project-name pgcqrs_deps down --volumes
  )
}
